...
class GuiButton {
	private _button: ImageButton | TextButton;
	...
	constructor(
		button: ImageButton,
		dumpster: Dumpster,
		customMotionOptions?: SpringOptions,
		config?: GuiButtonConfig,
	) {
		this.config = config;
		this._button = button;
		...
		if (!config?.useBaseButton) {
			this.deleteOriginalChildren();
		}
		this.init();
	}

	private deleteOriginalChildren() {
		for (const element of Object.values(this._button.GetChildren())) {
			if (element.IsA("GuiBase2d")) {
				element.Destroy();
			}
		}
	}
	...
	private init() {
		if (!this.config?.useBaseButton) {
			...
			if (this._button.IsA("ImageButton")) {
				this._button.ImageTransparency = 1;
			} else if (this._button.IsA("TextButton")) {
				this._button.TextTransparency = 1;
			}
		...
		this._dumpster.dump(
			this._buttonClone.MouseEnter.Connect(() => {
				this._motion.spring(this.config?.bounds?.max ?? 1.25, this._motionOptions);
			}),
		);

		this._dumpster.dump(
			this._buttonClone.MouseLeave.Connect(() => {
				this._motion.spring(this.config?.bounds?.min ?? 1, this._motionOptions);
			}),
		);

		this._dumpster.dump(
			this._buttonClone.Activated.Connect(() => {
				const impulseAmount = this.config?.impulse?.amount ?? 0.01;
		...
		this._dumpster.dump(
			this._motion.onStep((value, deltaTime) => {
				this._uiscale.Scale = value;
			}),
		);
	}

	public GetVisualButton() {
		return this._buttonClone as typeof this._button;
	}
}
